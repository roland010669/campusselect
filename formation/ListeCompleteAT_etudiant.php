<?php
$PageTitle = "listeCompleteTourismeAccueil-Étudiant";
...
$selLCATE = new Formation();
function FselLCATE($selLCATE)
{
  try {
    $connexion = new Connexion();
    $pdostat = $connexion->query("SELECT * FROM formation WHERE 	filiere='AT'AND niveau IN (3,4,5,6)");
    $pdostat->setFetchMode(PDO::FETCH_ASSOC);
    $selLCATE = $pdostat->fetchAll();
    return $selLCATE;
  } catch (Exception $e) {
    echo "ERREUR : " . $e->getMessage();
  }
}
        foreach (FselLCATE($selLCATE) as $f) : ?>
