<?php
$PageTitle = "listeCompleteHorticulturePaysage_Étudiant";
...
<table class="table table-bordered">
    ...
    <tbody>
        ...
        <?php
$selLCHTE = new Formation();
function FselLCHTE($selLCHTE)
{
  try {
    $connexion = new Connexion();
    $pdostat = $connexion->query("SELECT * FROM formation WHERE 	filiere='HT'AND niveau IN (3,4,5,6)");
    $pdostat->setFetchMode(PDO::FETCH_ASSOC);
    $selLCHTE = $pdostat->fetchAll();
    return $selLCHTE;
  } catch (Exception $e) {
    echo "ERREUR : " . $e->getMessage();
        ...
        foreach (FselLCHTE($selLCHTE) as $f) : ?>
