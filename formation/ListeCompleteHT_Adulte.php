<?php
$PageTitle = "listeCompleteHorticulturePaysage_Adulte";
...
<table class="table table-bordered">
    ...
    <tbody>
        ...
        <?php
$selLCHTA = new Formation();
function FselLCHTA($selLCHTA)
{
  try {
    $connexion = new Connexion();
    $pdostat = $connexion->query("SELECT * FROM formation WHERE 	filiere='HT'AND niveau IN (3,4,5,6,7)");
    $pdostat->setFetchMode(PDO::FETCH_ASSOC);
    $selLCHTA = $pdostat->fetchAll();
    return $selLCHTA;
  } catch (Exception $e) {
    echo "ERREUR : " . $e->getMessage();
        ...
        foreach (FselLCHTA($selLCHTA) as $f) : ?>
