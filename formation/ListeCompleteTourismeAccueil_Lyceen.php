<?php
$PageTitle = "listeCompleteTourismeAccueil-Lycéen";
...
$selLCATL = new Formation();
function FselLCATL($selLCATL)
{
  try {
    $connexion = new Connexion();
    $pdostat = $connexion->query("SELECT * FROM formation WHERE 	filiere='AT'AND niveau IN (3,4,5)");
    $pdostat->setFetchMode(PDO::FETCH_ASSOC);
    $selLCATL = $pdostat->fetchAll();
    return $selLCATL;
  } catch (Exception $e) {
    echo "ERREUR : " . $e->getMessage();
        ...
        foreach (FselLCATL($selLCATL) as $f) : ?>
