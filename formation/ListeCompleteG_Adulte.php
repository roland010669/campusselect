<?php
$PageTitle = "listeCompleteGastronomie_Hotellerie_Cuisine-Adulte";
...
<h1 style="text-shadow: 2px 2px 2px pink ;font-style: italic" class="text-center">Liste des Formations
    Gastronomie-Hotellerie-Cuisine-Adulte.
</h1>
...
$selLCGA = new Formation();
function FselLCGA($selLCGA)
{
  try {
    $connexion = new Connexion();
    $pdostat = $connexion->query("SELECT * FROM formation WHERE 	filiere='G'AND niveau IN ('3','4','5','6','7')");
    $pdostat->setFetchMode(PDO::FETCH_ASSOC);
    $selLCGA = $pdostat->fetchAll();
    return $selLCGA;
  } catch (Exception $e) {
