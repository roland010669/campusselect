<?php
$PageTitle = "listeComplete_Métiers_d'Art_et_du_design_Lyceen";
...
<h1 style="text-shadow: 2px 2px 2px pink ;font-style: italic" class="text-center">Liste des Formations
    Métiers d'Art et du design-Lycéen
</h1>
...
$selLADL = new Formation();
function FselLADL($selLADL)
{
  try {
    $connexion = new Connexion();
    $pdostat = $connexion->query("SELECT * FROM formation WHERE 	filiere='AD'AND niveau IN ('3','4','5')");
    $pdostat->setFetchMode(PDO::FETCH_ASSOC);
    $selLADL = $pdostat->fetchAll();
    return $selLADL;
  } catch (Exception $e) {
    echo "ERREUR : " . $e->getMessage();
  }
}
        foreach (FselLADL($selLADL) as $f) : ?>
