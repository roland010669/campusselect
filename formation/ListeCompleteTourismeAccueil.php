<?php
$PageTitle = "listeCompleteTourismeAccueil";
...
<table class="table table-bordered">
    ...
    <tbody>
        ...
        <?php
        $selLCAT = new Formation();
        function FselLCAT($selLCAT)
        {
            try {
                $connexion = new Connexion();
                $pdostat = $connexion->query("SELECT * FROM formation WHERE 	filiere='AT'AND niveau IN (3,4)");
                $pdostat->setFetchMode(PDO::FETCH_ASSOC);
                $selLCAT = $pdostat->fetchAll();
                return $selLCAT;
            } catch (Exception $e) {
