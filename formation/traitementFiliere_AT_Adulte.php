...
<head>
    ...
    <title>traitement_filièresAT_Adulte</title>
</head>

<body>
    <?php
    $PageTitle = "traitement_filièresAT_Adulte";
            ...
            if (count($tabsel) == 1) {
                ...
                if (in_array($tabsel[0], $tabDep)) {
                    try {
                        ...
                        $connexion = new Connexion();
                        $pdoStatement = $connexion->prepare("select * from formation where departement = :departement AND filiere='AT' AND niveau IN (3,4,5,6,7)");
