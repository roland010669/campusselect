<?php
$PageTitle = "listeCompleteHorticulturePaysage";
...
<table class="table table-bordered">
    ...
    <tbody>
        ...
        $selLCHT = new Formation();
        function FselLCHT($selLCHT)
        {
            try {
                $connexion = new Connexion();
                $pdostat = $connexion->query("SELECT * FROM formation WHERE 	filiere='HT'AND niveau IN (3,4)");
                $pdostat->setFetchMode(PDO::FETCH_ASSOC);
                $selLCHT = $pdostat->fetchAll();
                return $selLCHT;
            } catch (Exception $e) {
                echo "ERREUR : " . $e->getMessage();
            }
        }
        foreach (FselLCHT($selLCHT) as $f) : ?>
