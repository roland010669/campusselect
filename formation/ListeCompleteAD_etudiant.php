<?php
$PageTitle = "listeComplete_Métiers_d'Art_et_du_design_Étudiant";
...
<h1 style="text-shadow: 2px 2px 2px pink ;font-style: italic" class="text-center">Liste des Formations
    Métiers d'Art et du design-Étudiant
</h1>
<table class="table table-bordered">
    ...
    <tbody>
        ...
        <?php
        $selLADE = new Formation();
        function FselLADE($selLADE)
        {
            try {
                $connexion = new Connexion();
                $pdostat = $connexion->query("SELECT * FROM formation WHERE 	filiere='AD'AND niveau IN ('3','4','5','6')");
                $pdostat->setFetchMode(PDO::FETCH_ASSOC);
                $selLADE = $pdostat->fetchAll();
                return $selLADE;
            } catch (Exception $e) {
