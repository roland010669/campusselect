...
<head>
    ...
    <title>traitement_filièreGastronomie_Etudiant</title>
</head>

<body>
    <?php
    $PageTitle = "traitement_filièreGastronomie_Etudiant";
            ...
            if (count($tabsel) == 1) {
                ...
                if (in_array($tabsel[0], $tabDep)) {
                    try {
                        ...
                        $connexion = new Connexion();
                        $pdoStatement = $connexion->prepare("select * from formation where departement = :departement AND filiere='G' AND niveau IN (3,4,5,6)");
