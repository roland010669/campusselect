<?php
$PageTitle = "listeCompletePatrimoineBati_Adulte";
...
<table class="table table-bordered">
    ...
    <tbody>
        ...
        <?php
$selLCPBA = new Formation();
function FselLCPBA($selLCPBA)
{
  try {
    $connexion = new Connexion();
    $pdostat = $connexion->query("SELECT * FROM formation WHERE 	filiere='PB' AND niveau IN (3,4,5,6,7)");
    $pdostat->setFetchMode(PDO::FETCH_ASSOC);
    $selLCPBA = $pdostat->fetchAll();
    return $selLCPBA;
  } catch (Exception $e) {
    echo "ERREUR : " . $e->getMessage();
        ...
        foreach (FselLCPBA($selLCPBA) as $f) : ?>
