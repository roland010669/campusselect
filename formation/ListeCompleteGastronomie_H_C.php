<?php
$PageTitle = "listeCompleteGastronomie_Hotellerie_Cuisine";
...
<h1 style="text-shadow: 2px 2px 2px pink ;font-style: italic" class="text-center">Liste des Formations
    Gastronomie-Hotellerie-Cuisine
</h1>
<table class="table table-bordered">
    ...
    <tbody>
        ...
        <?php
        $selLCG = new Formation();
        function FselLCG($selLCG)
        {
            try {
                $connexion = new Connexion();
                $pdostat = $connexion->query("SELECT * FROM formation WHERE 	filiere='G'AND niveau IN ('3','4')");
                $pdostat->setFetchMode(PDO::FETCH_ASSOC);
                $selLCG = $pdostat->fetchAll();
                return $selLCG;
            } catch (Exception $e) {
