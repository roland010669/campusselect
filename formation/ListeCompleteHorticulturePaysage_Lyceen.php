<?php
$PageTitle = "listeCompleteHorticulturePaysage_Lycéen";
...
$selLCHTL = new Formation();
function FselLCHTL($selLCHTL)
{
  try {
    $connexion = new Connexion();
    $pdostat = $connexion->query("SELECT * FROM formation WHERE 	filiere='HT'AND niveau IN (3,4,5)");
    $pdostat->setFetchMode(PDO::FETCH_ASSOC);
    $selLCHTL = $pdostat->fetchAll();
    return $selLCHTL;
  } catch (Exception $e) {
    echo "ERREUR : " . $e->getMessage();
        ...
        foreach (FselLCHTL($selLCHTL) as $f) : ?>
