<?php
$PageTitle = "listeComplete_Métiers_d'Art_et_du_design";
...
<table class="table table-bordered">
    ...
    <tbody>
        ...
        $selLAD = new Formation();
        function FselLAD($selLAD)
        {
            try {
                $connexion = new Connexion();
                $pdostat = $connexion->query("SELECT * FROM formation WHERE 	filiere='AD'AND niveau IN ('3','4')");
                $pdostat->setFetchMode(PDO::FETCH_ASSOC);
                $selLAD = $pdostat->fetchAll();
                return $selLAD;
            } catch (Exception $e) {
                echo "ERREUR : " . $e->getMessage();
            }
        }
        foreach (FselLAD($selLAD) as $f) : ?>
